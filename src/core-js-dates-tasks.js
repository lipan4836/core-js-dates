...
function getNextFriday(date) {
  ...
  const gap = day === 5 ? 7 : (5 - day + 7) % 7;
  return new Date(date.setDate(date.getDate() + gap));
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (new Date(dateEnd) - new Date(dateStart)) / dayInMs + 1;
}
...
function getWeekNumberByDate(date) {
  ...
  startOfWeek.setDate(jan1.getDate() - daysOffset + (daysOffset > 0 ? 7 : 0));
  return Math.ceil((date - startOfWeek) / week) + 1;
}
...
function getNextFridayThe13th(date) {
  const fridayThe13 = new Date(date);
  fridayThe13.setDate(13);

  while (fridayThe13.getDay() !== 5) {
    fridayThe13.setMonth(fridayThe13.getMonth() + 1);
  }
